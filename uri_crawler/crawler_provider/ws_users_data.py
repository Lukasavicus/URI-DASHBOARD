...
def get_num_pages(users_module_address=USERS_MODULE_ADDRESS):
    ...
    try:
        print(re.search('[0-9]+(,|\.|)[0-9]+$', num_pages_text))
        num_pages_filtered = re.search('[0-9]+(,|\.|)[0-9]+$', num_pages_text).group(0)
        print(num_pages_filtered)
        num_pages_filtered = string.replace(num_pages_filtered, ",", "")
        print(num_pages_filtered)
        num_pages_filtered = string.replace(num_pages_filtered, ".", "")
        print(num_pages_filtered)
        num_pages = int(num_pages_filtered)
...
def craw_users(username, limit=100):
    print("craw_users()")
    USERS = []
    num_pages = get_num_pages();
    if limit > 0:
        num_pages = limit
    print("Algorithm will check " + str(num_pages) + " pages.")
